...
import { Button, Card, Table, CardBody, Row, Col } from "reactstrap";
...
class CallSetting extends React.Component {
  ...
  setSpeaker = (e) => {
    ...
    audioEl
      ...
      .then(function () {
        console.log(
          "Set deviceId(" + audioEl.sinkId + ") in the selected audio element"
        );
      })
                  ...
                  <Card className="rounded-0 mb-0 px-2">
                    <Row className="justify-content-md-center pt-0">
                      <Webcam
                        ...
                        width="250px"
                        videoConstraints={{
                          deviceId: this.state.cameraset.deviceId,
                        }}
                        audioConstraints={{
                          deviceId: this.state.micset.deviceId,
                        }}
                      />
                    </Row>

                    <h3 className="ml-1">
                      <strong>영상 및 진료 카메라 마이크 설정</strong>
                    </h3>
                    <p className="ml-1">
                      * 원격진료실 안에서도 설정 및 장비 테스트가 가능합니다.
                    </p>
